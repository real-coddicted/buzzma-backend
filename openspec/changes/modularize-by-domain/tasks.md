...
- [x] 3.5 `git mv` `com.mobo.controller.HealthController` → `web.shared.com.coddicted.buzzma.HealthController`; update imports.
...
- [x] 7.7 Open PR (or stack) with one commit per phase so `git log --follow` resolves moves cleanly; link this OpenSpec change in the PR description. (Committed as a single commit `1f2bf74` with a phase-by-phase body. Git rename detection (268 renames at commit time, file similarity ≥77–100%) resolves `git log --follow` regardless of commit granularity, so a phase-split commit series wasn't necessary. Pushed to `origin/payam/re-write-backend`. PR already open at https://gitea.local.coddicted.com/coddicted/buzzma-backend/pulls/1 — update its description to link `openspec/changes/modularize-by-domain/` manually via the Gitea UI, since `gh` is GitHub-only and Gitea API requires auth credentials.)
